...
/** Animate items along the y-axis in an endless loop */
export default function verticalLoop(
  targets: gsap.DOMTarget | Element[],
  config?: VerticalLoopConfig
): gsap.core.Timeline {
  const items = gsap.utils.toArray<HTMLElement>(targets as unknown as string);
  const cfg = config || {};
  ...
  gsap.set(items, {
    yPercent: (i: number, el: HTMLElement) => {
      const h = (heights[i] = parseFloat(gsap.getProperty(el, 'height', 'px') as string));
      yPercents[i] = snap(
  ...
  const totalHeight =
    ...
    startY +
    lastItem.offsetHeight * (gsap.getProperty(lastItem, 'scaleY') as number) +
    (parseFloat(String(cfg.paddingBottom)) || 0);
